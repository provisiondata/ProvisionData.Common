...
public sealed partial class CustomErrorSourceGenerator/*(ILogger logger)*/ : IIncrementalGenerator
{
    ...
    private static INamedTypeSymbol? GetErrorSymbol(GeneratorSyntaxContext ctx)
    {
        ...
        while (baseType is not null)
        {
            ...
            if (fullName == RPConsts.FullyQualifiedErrorTypeName)
            {
                return symbol;
            }
    ...
    private static void ErrorCodesGenerator(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> errors)
    {
        ...
        foreach (var error in errors)
        {
            ...
            var codeName = errorName + RPConsts.ErrorCodeSuffix;
            ...
            sb.AppendLine($"    internal sealed class {codeName} : {RPConsts.ResultPatternNamespace}.ErrorCode");
            sb.AppendLine("    {");
        ...
        context.AddSource(RPConsts.GeneratedErrorCodesFilename, sb.ToString());
    }
    ...
    private static void ErrorConstructorsGenerator(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> errors)
    {
        ...
        context.AddSource(RPConsts.GeneratedErrorConstructorsFilename, sb.ToString());
    }
    ...
    private static void ErrorTypeRegistryGenerator(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> errors)
    {
        ...
        sb.AppendLine("#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member");
        sb.AppendLine($"    internal static class {RPConsts.ErrorTypeRegistryInitializerClassName}");
        sb.AppendLine("    {");
        sb.AppendLine("        [System.Runtime.CompilerServices.ModuleInitializer]");
        sb.AppendLine("        internal static void Initialize()");
        sb.AppendLine("        {");
        sb.AppendLine("            Console.WriteLine(\"Initializing ErrorTypeRegistry...\");");

        foreach (var error in errors)
        {
            var fullName = error.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)
                .Replace("global::", "");
            sb.AppendLine($"            {RPConsts.ResultPatternNamespace}.Infrastructure.{RPConsts.ErrorTypeRegistryClassName}.Register<{fullName}>();");
        }
        ...
        context.AddSource(RPConsts.ErrorTypeRegistryInitializerFilename, sb.ToString());
    }
    ...
    private static void ErrorPolymorphismHookGenerator(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> errors)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"internal static class {RPConsts.ErrorPolymorphismInitializerClassName}");
        sb.AppendLine("{");
        sb.AppendLine("    [ModuleInitializer]");
        sb.AppendLine("    internal static void Initialize()");
        sb.AppendLine("    {");
        sb.AppendLine("        Console.WriteLine(\"Initializing ErrorPolymorphism...\");");
        sb.AppendLine($"        {RPConsts.ResultPatternNamespace}.Infrastructure.{RPConsts.ErrorJsonPolymorphismClassName}.Register(Configure);");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    private static void Configure(JsonTypeInfo ti)");
        sb.AppendLine("    {");
        sb.AppendLine($"        if (ti.Type == typeof({RPConsts.ResultPatternNamespace}.ErrorCode))");
        sb.AppendLine("        {");
        sb.AppendLine("            ti.PolymorphismOptions ??= new JsonPolymorphismOptions");
        sb.AppendLine("            {");
        sb.AppendLine($"                {RPConsts.TypeDiscriminatorPropertyName} = \"$code\"");
        sb.AppendLine("            };");
        ...
        context.AddSource(RPConsts.ErrorPolymorphismInitializerFilename, sb.ToString());
    }
