...
public sealed class ResultPatternConfigurationAnalyzer : DiagnosticAnalyzer
{
    ...
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics => [ResultPatternDiagnosticDefinitions.MissingAddResultPattern];
    ...
    private static void Start(CompilationStartAnalysisContext context)
    {
        ...
        context.RegisterSyntaxNodeAction(syntaxContext =>
        {
            ...
            {
                if (typeSymbol.ContainingNamespace.ToDisplayString().StartsWith(RPConsts.ResultPatternNamespace))
                {
        ...
        context.RegisterCompilationEndAction(endContext =>
        {
            if (usedResultPatternTypes && !addResultPatternCalled)
            {
                ...
                var diagnostic = Diagnostic.Create(ResultPatternDiagnosticDefinitions.MissingAddResultPattern, location);
