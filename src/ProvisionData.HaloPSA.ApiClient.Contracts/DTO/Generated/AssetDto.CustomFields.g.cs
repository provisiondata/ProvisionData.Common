...
/// <summary>
/// Custom fields implementation for AssetDto.
/// </summary>
public partial class AssetDto
{
    ...
    public static void ApplyFieldMappings(IFieldMappingProvider provider)
    {
        if (!Mapped)
        {
            lock (MappingLock)
            {
                if (!Mapped)
                {
                    ModelFieldId = provider.GetFieldId(nameof(AssetDto), nameof(Model));
                    NameFieldId = provider.GetFieldId(nameof(AssetDto), nameof(Name));
    ...
    /// <inheritdoc/>
    public String ModelTypeName => nameof(AssetDto);
