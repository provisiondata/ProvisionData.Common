    ...
    : ResultPatternUnitTestBase(fixture, output)
{
    ...
    {
        var result = Result<Int32>.Failure(new CustomError("Not found"));
    ...
    {
        var result = Result<String>.Failure(new CustomError("Not found"));
    ...
    public void Map_WithFailureResult_ShouldReturnFailure()
    {
        ...
        mapped.Error.Should().BeOfType<CustomError>();
    }
    ...
    public void Bind_WithSuccessResult_ShouldChainOperation()
    {
        ...
        var bound = result.Bind(x =>
            x > 0 ? Result<String>.Success(x.ToString()) : new CustomError("On vacation"));
    ...
    public void Bind_WithFailureInBinder_ShouldReturnBinderFailure()
    {
        ...
        var bound = result.Bind(x =>
            x > 0 ? Result<String>.Success(x.ToString()) : new CustomError("Ask nicely next time"));

        bound.IsSuccess.Should().BeFalse();
        bound.Error.Should().BeOfType<CustomError>();
    }
    ...
    {
        var result = Result<Int32>.Failure(new CustomError("Item not found"));
    ...
    {
        var result = Result.Failure(new CustomError("Not found"));

        var typedResult = result.ToResult(42);

        typedResult.IsSuccess.Should().BeFalse();
        typedResult.Error.Should().BeOfType<CustomError>();
    }
    ...
    public void Chaining_ShouldWorkCorrectly()
    {
        var result = Result<Int32>.Success(5)
            .Map(x => x * 2)
            .Bind(x => x > 5 ? Result<String>.Success(x.ToString()) : new CustomError("Missing"))
