    ...
    : ResultPatternUnitTestBase(fixture, output)
{
    ...
    {
        Result<TestRecord> original = new CustomError("Customer not found");
        var json = JsonSerializer.Serialize(original);
        ...
        deserialized.Error.Should().BeOfType<CustomError>();
    ...
    {
        var original = new CustomError("Resource ID 123 not found");
        var json = JsonSerializer.Serialize(original);

        var deserialized = JsonSerializer.Deserialize<CustomError>(json);

        deserialized.Should().NotBeNull();
        deserialized!.Description.Should().Be("Resource ID 123 not found");
        deserialized.Code.Should().Be(new CustomError("test").Code);
    }

    [Fact]
    public void Error_WithSpecialCharacters_ShouldRoundTrip()
    {
        var original = new CustomError("Path: <>&\"'\\\n\t");
        var json = JsonSerializer.Serialize(original);
    ...
    {
        Error original = new CustomError("Item not found");
        var json = JsonSerializer.Serialize(original);

        var deserialized = JsonSerializer.Deserialize<Error>(json);

        deserialized.Should().NotBeNull();
        deserialized.Should().BeOfType<CustomError>();
    ...
    {
        Result<Int32> original = new CustomError("Failed");
        var json = JsonSerializer.Serialize(original);
        var deserialized = JsonSerializer.Deserialize<Result<Int32>>(json);
    ...
    {
        var original = new CustomError("test");
        var json = JsonSerializer.Serialize(original);
        var deserialized = JsonSerializer.Deserialize<CustomError>(json);

        deserialized!.IsErrorType<CustomError>().Should().BeTrue();
        deserialized.IsErrorType<CustomError>().Should().BeFalse();
    }
    ...
    {
        Result<String> result = new CustomError("Not found");
    ...
    {
        Result<Int32> result = new CustomError("Conflict");
        var json = JsonSerializer.Serialize(result);
    ...
    public void ResultOfT_IsSuccess_ShouldMatchAfterRoundTrip()
    {
        ...
        Result<Int32> failure = new CustomError("Not found");
