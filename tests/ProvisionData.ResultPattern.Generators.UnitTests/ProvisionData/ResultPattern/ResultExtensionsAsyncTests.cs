    ...
    : ResultPatternUnitTestBase(fixture, output)
{
    ...
    public async Task BindAsync_WithSuccessResult_ShouldChainOperation()
    {
        ...
        {
            await Task.Delay(1);
            return x > 0 ? Result<String>.Success(x.ToString()) : new CustomError("Kidnapped");
        });
    ...
    public async Task BindAsync_WithTask_ShouldChainOperation()
    {
        ...
        var bound = await resultTask.BindAsync(x =>
            x > 0 ? Result<String>.Success(x.ToString()) : new CustomError("Gone Fishing"));
    ...
    public async Task BindAsync_WithTaskAndAsyncBinder_ShouldChainOperation()
    {
        ...
        {
            await Task.Delay(1);
            return x > 0 ? Result<String>.Success(x.ToString()) : new CustomError("Smoke Break");
        });
    ...
    public async Task AsyncChaining_ShouldWorkCorrectly()
    {
        var result = await Task.FromResult(Result<Int32>.Success(5))
            ...
            {
                await Task.Delay(1);
                return x > 5 ? Result<String>.Success(x.ToString()) : new CustomError("Sick - Have Doctor's Note");
            })
            .TapAsync(async x =>
            {
                await Task.Delay(1);
